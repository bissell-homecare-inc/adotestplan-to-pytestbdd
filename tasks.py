...
def delete_test_work_items(c):
    ...
    connection = Connection(
        base_url=test_url, creds=credentials)
    ...
    if input(f"are you sure you want to delete ALL test-related work items from within the \"{proj}\" project under {test_url} (y/n)") != "y":  # noqa: E501
        return
    ...
    wiql.query = 'Select [System.Id] From WorkItems Where [System.WorkItemType] = \'Test Plan\''  # noqa: E501
    ...
    wiql.query = 'Select [System.Id] From WorkItems Where [System.WorkItemType] = \'Test Suite\''  # noqa: E501
    ...
    wiql.query = 'Select [System.Id] From WorkItems Where [System.WorkItemType] = \'Test Case\''  # noqa: E501
    for tc in witc.query_by_wiql(wiql).work_items:
        tpc.delete_test_case(proj, tc.id)

    wiql.query = 'Select [System.Id] From WorkItems Where [System.WorkItemType] = \'Shared Steps\''  # noqa: E501
    for shared_step in witc.query_by_wiql(wiql).work_items:
        ...
        headers['Authorization'] = 'Basic ' + \
            base64.b64encode(str.encode(f'user:{pat}')).decode('utf-8')
        response = requests.delete(url=url, headers=headers)
        if response.status_code != 204:
            print(
                f'Issue deleting shared step {shared_step.id} returned: {response.status_code} : {response.reason}')  # noqa: E501
...
def generate_test_work_items(c):
    ...
    connection = Connection(
        base_url=url, creds=credentials)
    clients = connection.clients
            ...
            "suites": {
                "Shared Steps and Shared Params Suite": {
                    "Shared Param and Shared Step Scenario": {
                        ...
                        "steps": [
                            "When a non shared step",
                            "Then @Parameter1"
                        ],
                        "shared parameters": {
                            "Shared Parameters 1": ["Parameter1"]
                        }
                    },
    ...
    # to them
    shared_parameters = populate_shared_parameters(
        shared_parameters, witc, proj)
    ...
    populate_test_plans(test_plans, tpc, witc, proj,
                        shared_parameters, shared_step_ids)


def populate_test_plans(test_plans, tpc, witc, proj, shared_parameters, shared_step_ids):
    from azure.devops.v7_0.test_plan.models import (
        SuiteTestCaseCreateUpdateParameters, TestPlan, TestPlanCreateParams,
        TestSuite, TestSuiteCreateParams)
    ...
    for title, tp in test_plans.items():
        ...
        for suite_title, suite in tp['suites'].items():
            ...
            for scenario_title, scenario in suite.items():
                ...
                test_case_work_item = witc.create_work_item(
                    document, proj, "Test Case")
...
def populate_shared_steps(shared_steps, witc, proj):
    ...
    for title, shared_step_value in shared_steps.items():
        ...
        shared_step_work_item = witc.create_work_item(
            document, proj, "Shared Steps")
        if isinstance(shared_step_value, list):
            ...
            for step in shared_step_value:
                ...
                second_parameterized_string_element = Element(
                    'parameterizedString')
                second_parameterized_string_element.attrib['isFormatted'] = 'true'
                second_parameterized_string_element.text = '<DIV><DIV><P><BR/></P></DIV></DIV>'  # noqa: E501
...
def populate_shared_parameters(shared_parameters, witc, proj):
    ...
    for title, shared_parameter in shared_parameters.items():
        ...
        shared_parameters[title]['id'] = witc.create_work_item(
            document, proj, "Shared Parameter").id
...
def populate_parameters_for_scenario(scenario, document, shared_parameters):
    ...
    if parameters_element:
        ...
        document.append(
            JsonPatchOperation(
                op="add", path="/fields/Microsoft.VSTS.TCM.Parameters",
                value=parameter_name_payload
            )
        )
        parameter_data_payload = tostring(
            parameter_data_element).decode('utf-8')
        document.append(
            JsonPatchOperation(
    ...
    if 'shared parameters' in scenario and len(scenario['shared parameters']):
        ...
        lds = {"parameterMap": [], "rowMappingType": 0,
               "sharedParameterDataSetIds": []}
        parameters_element = Element('parameters')
...
def populate_steps_for_scenario(scenario, document, shared_step_ids):
    ...
    if 'shared steps' in scenario and len(scenario['shared steps']):
        for step in scenario['shared steps']:
            ...
            compref_element.attrib['ref'] = str(
                shared_step_ids[step])
